                       ...
                       int negative, MPI_Comm comm)
{
    ...
    /* At the first step, perp_disp is zero vector
       because disp_vector and push_direction are parallel. */
    double *disp_direction;
    if (sps_step == 1) {
        disp_direction = (double *)calloc(active_num * 3, sizeof(double));
    } else {
        double *perp_disp = perpendicular_vector(disp_vector, push_direction, active_num);
        disp_direction = normalize(perp_disp, active_num);
        free(perp_disp);
    }
    free(disp_vector);
                ...
                double *Ea, MPI_Comm comm)
{
    ...
    if (input->init_disp > 0) {
        ...
        /* not all atoms in disp_cutoff are fixed */
        if (num_fix < tmp_num) {
