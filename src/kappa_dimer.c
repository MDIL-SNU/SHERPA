...
static void rotate(Config *config0, double energy0, double *force0,
                   Input *input, int local_num, int *local_list,
                                 ...
                                 double *eigenmode, MPI_Comm comm)
{
    ...
    double *force1 = (double *)malloc(sizeof(double) * local_num * 3);
    double *force2 = (double *)malloc(sizeof(double) * local_num * 3);
    double *unit_force0 = normalize(force0, local_num);
    for (i = 0; i < input->max_num_rot; ++i) {
        /* let eigenmode normal to force */
        tmp_eigenmode = perpendicular_vector(eigenmode, unit_force0, local_num);
        new_eigenmode = normalize(tmp_eigenmode, local_num);
        for (j = 0; j < local_num; ++j) {
            eigenmode[j * 3 + 0] = new_eigenmode[j * 3 + 0];
            eigenmode[j * 3 + 1] = new_eigenmode[j * 3 + 1];
            eigenmode[j * 3 + 2] = new_eigenmode[j * 3 + 2];
        ...
        double *new_f_rot_A = perpendicular_vector(f_rot_A, unit_force0, local_num);
        for (j = 0; j < local_num; ++j) {
        ...
        /* derivative of curvature */
        oneshot_local(trial_config1, input, &energy1, force1, local_num, local_list, comm);
        free_config(trial_config1);
        for (j = 0; j < local_num; ++j) {
            force2[j * 3 + 0] = 2 * force0[j * 3 + 0] - force1[j * 3 + 0];
            force2[j * 3 + 1] = 2 * force0[j * 3 + 1] - force1[j * 3 + 1];
        ...
        double rotangle = 0.5 * atan(b1 / a1);
        cmin = 0.5 * a0 + a1 * cos(2 * rotangle) + b1 * sin(2 * rotangle);
        kappa = -cmin / norm(force0, local_num);
        if (c0 < cmin) {
            rotangle += 3.1415926535897932384626 * 0.5;
        }
        double *tmp_force;
                      ...
                      int dimer_step, double kappa, MPI_Comm comm)
{
    ...
    /* projected force */
    double *f0p = projected_force(force0, eigenmode, kappa, local_num);
    /* cg_direction */
                ...
                MPI_Comm comm)
{
    ...
    for (dimer_step = 1; dimer_step <= input->max_num_tls; ++dimer_step) {
        ...
        /* kappa-dimer */
        double *tmp_eigenmode = (double *)malloc(sizeof(double) * local_num * 3);
        for (i = 0; i < local_num; ++i) {
            tmp_eigenmode[i * 3 + 0] = eigenmode[i * 3 + 0];
            tmp_eigenmode[i * 3 + 1] = eigenmode[i * 3 + 1];
            tmp_eigenmode[i * 3 + 2] = eigenmode[i * 3 + 2];
        }
        kappa = constrained_rotate(config0, force0, input, local_num, local_list,
                                   tmp_eigenmode, comm);
        free(tmp_eigenmode);
